...
static void draw_buttons(Runner &r)
{
	for (size_t i = 0; i < r.doc.buttons.size(); i++) {
		...
		if (ImGui::Button(label.c_str(), ImVec2(150, 48))) r.press(b.name);
		ImGui::EndDisabled();
		ImGui::PopStyleColor(2);
		ImGui::PopID();
		// wrap to the next line when the row is full
		float next_x = ImGui::GetItemRectMax().x + ImGui::GetStyle().ItemSpacing.x + 150;
...
static void draw_popups(Runner &r)
{
	...
	if (ImGui::BeginPopupModal("Question", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		if (ImGui::Button("Yes", ImVec2(110, 0))) { r.ask_answer = 1; ImGui::CloseCurrentPopup(); }
		ImGui::SameLine();
		if (ImGui::Button("No", ImVec2(110, 0))) { r.ask_answer = 0; ImGui::CloseCurrentPopup(); }
		ImGui::EndPopup();
	}
	...
	if (ImGui::BeginPopupModal("Message", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		if (ImGui::Button("OK", ImVec2(110, 0))) { r.popup_text.clear(); ImGui::CloseCurrentPopup(); }
		ImGui::EndPopup();
	}
	...
	if (ImGui::BeginPopupModal("Error", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		if (ImGui::Button("OK", ImVec2(110, 0))) { r.error_text.clear(); ImGui::CloseCurrentPopup(); }
		ImGui::EndPopup();
	}
	ImGui::PopStyleColor();
}
...
static void draw_log(Proc &p)
{
	...
	ImGui::BeginChild("log", ImVec2(0, 200), ImGuiChildFlags_Borders, ImGuiWindowFlags_HorizontalScrollbar);
